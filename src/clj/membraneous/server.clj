(ns membraneous.server
  ...
  (:require [clojure.edn :as edn] 
            ...
            [membraneous.core :as membraneous]))
...
(defn stop-channel
  [clients channel]
  (membraneous/halt-skeletons (get-in clients [channel :every]))
      ...
      (httpkit/on-receive 
       ...
       (fn [raw]
         ...
         (when-not (get @clients channel)
           (let [every (membraneous/provide-skeletons (send-skeletons channel) 50)]
...
(defn -main 
  ...
  (let [port 19991]
    ...
    (membraneous/wake-skeletons)
    (println (format "membraneous started on %s" port))))
